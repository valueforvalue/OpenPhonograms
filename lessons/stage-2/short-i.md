# Lesson 2: Short Vowel I — /ĭ/

**Stage 2** · Lesson 2 · vowel-concept

---

## Warm-Up: Phonogram Flash Review

> Flash all 26 a-z phonograms. Child says ALL sounds within 2 seconds. Focus especially on **i**.

| Review all a-z |
|----------------|
| a, d, g, c, o, qu, s, t, i, p, u, j, r, n, m, e, l, b, h, k, f, v, w, x, y, z |

---

## New Learning: The Short Sound of **I**

### Meet /ĭ/

**I** has multiple sounds. The short sound of **i** is /ĭ/.

The short sound of I is /ĭ/ — like the first sound in 'igloo.' Your mouth is slightly open, your tongue is high in the front. Most children find /ĭ/ easier to say than /ĕ/. Say it: /ĭ/ (short and crisp!).

<div class="phonogram">i</div>

### The Breve Mark ˘

When i says its short sound /ĭ/, we can mark it with a **breve** (˘) — a little smile above the letter. This helps us remember it's the short sound.

> ī  = long &nbsp;&nbsp;|&nbsp;&nbsp; ĭ = short (breve)

### Short I Words

| Word | Sounds | Meaning |
|------|--------|---------|
| sit | /s/ /i/ /t/ | — |
| fit | /f/ /i/ /t/ | — |
| hit | /h/ /i/ /t/ | — |
| bit | /b/ /i/ /t/ | — |
| pit | /p/ /i/ /t/ | — |
| lit | /l/ /i/ /t/ | — |
| kit | /k/ /i/ /t/ | — |
| wit | /w/ /i/ /t/ | — |

---

## Spelling Analysis

| Word | Phonograms Used | Rules Applied | Say-to-Spell |
|------|----------------|---------------|--------------|
| sit | s (/s/), i (/ĭ/), t (/t/) | Short vowel — closed syllable | /sit/ |
| fit | f (/f/), i (/ĭ/), t (/t/) | Short vowel — closed syllable | /fit/ |
| hit | h (/h/), i (/ĭ/), t (/t/) | Short vowel — closed syllable | /hit/ |

> **At this stage:** The teacher may need to help by saying each sound slowly. Child writes one phonogram at a time, saying the sound as they write.

---

## Reading Practice

Read these words sound by sound, then blend:

> sit &nbsp;&nbsp; fit &nbsp;&nbsp; hit &nbsp;&nbsp; bit &nbsp;&nbsp; pit &nbsp;&nbsp; lit &nbsp;&nbsp; kit &nbsp;&nbsp; wit &nbsp;&nbsp; it &nbsp;&nbsp; in &nbsp;&nbsp; if &nbsp;&nbsp; is &nbsp;&nbsp; big &nbsp;&nbsp; dig &nbsp;&nbsp; fig &nbsp;&nbsp; pig &nbsp;&nbsp; rig &nbsp;&nbsp; wig &nbsp;&nbsp; jig &nbsp;&nbsp; him &nbsp;&nbsp; rim &nbsp;&nbsp; dim &nbsp;&nbsp; Tim &nbsp;&nbsp; Jim &nbsp;&nbsp; pin &nbsp;&nbsp; fin &nbsp;&nbsp; tin &nbsp;&nbsp; win &nbsp;&nbsp; bin &nbsp;&nbsp; sin &nbsp;&nbsp; tip &nbsp;&nbsp; hip &nbsp;&nbsp; lip &nbsp;&nbsp; rip &nbsp;&nbsp; sip &nbsp;&nbsp; zip

---

## Dictation

Adult reads these sentences aloud. Child writes them.

> The sit is big.

---

## Quick Check

1. What is the short sound of **i**? *(/ĭ/)*
2. What mark do we use to show a short vowel? *(A breve ˘ — like a smile)*
3. Say a word with short /ĭ/ that we haven't used today.

---

**Next lesson:** Lesson 3: Short Vowel o

---

*Practice at home: Find 5 things in your house with the short /ĭ/ sound.*


