# Lesson 24: Mid-Stage 2 Assessment

**Stage 2** · Lesson 24 · assessment

---

## Assessment Overview

This mid-point check verifies the child is on track. Focus on CVC accuracy, first multi-letter phonograms, and blending with blends.

---

## Part 1: Phonogram Sounds

> Flash each phonogram card. Child says ALL sounds in frequency order within 2 seconds.

| Phonogram | Sounds | ✓ / Needs Work |
|-----------|--------|----------------|
| a | /ă/ /ā/ /ä/ | ☐ |
| b | /b/ | ☐ |
| c | /k/ /s/ | ☐ |
| d | /d/ | ☐ |
| e | /ĕ/ /ē/ | ☐ |
| f | /f/ | ☐ |
| g | /g/ /j/ | ☐ |
| h | /h/ | ☐ |
| i | /ĭ/ /ī/ /ē/ | ☐ |
| j | /j/ | ☐ |
| k | /k/ | ☐ |
| l | /l/ | ☐ |
| m | /m/ | ☐ |
| n | /n/ | ☐ |
| o | /ŏ/ /ō/ /ö/ | ☐ |
| p | /p/ | ☐ |
| qu | /kw/ | ☐ |
| r | /r/ | ☐ |
| s | /s/ /z/ | ☐ |
| t | /t/ | ☐ |
| u | /ŭ/ /ū/ /ö/ | ☐ |
| v | /v/ | ☐ |
| w | /w/ | ☐ |
| x | /ks/ /z/ | ☐ |
| y | /y/ /ĭ/ /ī/ /ē/ | ☐ |
| z | /z/ | ☐ |
| sh | /sh/ | ☐ |
| th | /th/ (voiced) /th/ (unvoiced) | ☐ |
| ck | /k/ | ☐ |
| ee | /ē/ | ☐ |
| ng | /ng/ | ☐ |
| ar | /är/ | ☐ |
| or | /or/ | ☐ |
| er | /er/ | ☐ |

**Score:** __ / 34

---

## Part 2: Word Reading

> Child reads each word aloud. Sound-by-sound blending is OK.

| Word | ✓ |
|------|---|
| cat | ☐ |
| ship | ☐ |
| back | ☐ |
| see | ☐ |
| stop | ☐ |
| hand | ☐ |
| sing | ☐ |
| car | ☐ |
| for | ☐ |
| her | ☐ |

**Score:** __ / 10

---

## Part 3: Spelling (Dictation)

> Adult dictates each word. Child writes it independently.

| Word | ✓ |
|------|---|
| cat | ☐ |
| ship | ☐ |
| back | ☐ |
| see | ☐ |
| stop | ☐ |
| that | ☐ |
| sing | ☐ |
| car | ☐ |

**Score:** __ / 8

---

## Part 4: Rule Knowledge

> Adult asks the question. Child explains in their own words.

| Rule | Question | ✓ / Needs Work |
|------|----------|----------------|
| Rule 26 | When do we use CK instead of K? | ☐ |
| Short vowels | Name the five short vowel sounds. | ☐ |

**Score:** __ / 2

---

## Results

| Section | Score | Pass? |
|---------|-------|-------|
| Phonogram Sounds | __ / 34 | |
| Word Reading | __ / 10 | |
| Spelling | __ / 8 | |
| Rule Knowledge | __ / 2 | |

**Overall:** __ / 54

---

## Next Steps

If ≥85%: Continue to second half of Stage 2. If weaker, review trouble spots for 1-2 weeks and retest.

---

*Great work! Every assessment shows what you've learned and what to practice next.*
