# Lesson 6: Short Vowel E — /ĕ/

**Stage 2** · Lesson 6 · vowel-concept

---

## Warm-Up: Phonogram Flash Review

> Flash all 26 a-z phonograms. Child says ALL sounds within 2 seconds. Focus especially on **e**.

| Review all a-z |
|----------------|
| a, d, g, c, o, qu, s, t, i, p, u, j, r, n, m, e, l, b, h, k, f, v, w, x, y, z |

---

## New Learning: The Short Sound of **E**

### Meet /ĕ/

**E** has multiple sounds. The short sound of **e** is /ĕ/.

The short sound of E is /ĕ/ — like the first sound in 'egg' or 'elephant.' Your mouth is open a little. This is often the hardest short vowel for children, so practice it often! Think of a creaky door: 'ehhh.' Say it: /ĕ/.

<div class="phonogram">e</div>

### The Breve Mark ˘

When e says its short sound /ĕ/, we can mark it with a **breve** (˘) — a little smile above the letter. This helps us remember it's the short sound.

> ē  = long &nbsp;&nbsp;|&nbsp;&nbsp; ĕ = short (breve)

### Short E Words

| Word | Sounds | Meaning |
|------|--------|---------|
| bed | /b/ /e/ /d/ | — |
| red | /r/ /e/ /d/ | — |
| fed | /f/ /e/ /d/ | — |
| led | /l/ /e/ /d/ | — |
| wed | /w/ /e/ /d/ | — |
| Ted | /T/ /e/ /d/ | — |
| Ned | /N/ /e/ /d/ | — |
| get | /g/ /e/ /t/ | — |

---

## Spelling Analysis

| Word | Phonograms Used | Rules Applied | Say-to-Spell |
|------|----------------|---------------|--------------|
| bed | b (/b/), e (/ĕ/), d (/d/) | Short vowel — closed syllable | /bed/ |
| red | r (/r/), e (/ĕ/), d (/d/) | Short vowel — closed syllable | /red/ |
| fed | f (/f/), e (/ĕ/), d (/d/) | Short vowel — closed syllable | /fed/ |

> **At this stage:** The teacher may need to help by saying each sound slowly. Child writes one phonogram at a time, saying the sound as they write.

---

## Reading Practice

Read these words sound by sound, then blend:

> bed &nbsp;&nbsp; red &nbsp;&nbsp; fed &nbsp;&nbsp; led &nbsp;&nbsp; wed &nbsp;&nbsp; Ted &nbsp;&nbsp; Ned &nbsp;&nbsp; get &nbsp;&nbsp; set &nbsp;&nbsp; let &nbsp;&nbsp; met &nbsp;&nbsp; pet &nbsp;&nbsp; vet &nbsp;&nbsp; wet &nbsp;&nbsp; jet &nbsp;&nbsp; net &nbsp;&nbsp; bet &nbsp;&nbsp; yet &nbsp;&nbsp; hen &nbsp;&nbsp; pen &nbsp;&nbsp; ten &nbsp;&nbsp; men &nbsp;&nbsp; den &nbsp;&nbsp; Ben &nbsp;&nbsp; Ken &nbsp;&nbsp; Len &nbsp;&nbsp; leg &nbsp;&nbsp; peg &nbsp;&nbsp; beg &nbsp;&nbsp; Meg &nbsp;&nbsp; peg &nbsp;&nbsp; hem &nbsp;&nbsp; gem &nbsp;&nbsp; them

---

## Dictation

Adult reads these sentences aloud. Child writes them.

> The bed is big.

---

## Quick Check

1. What is the short sound of **e**? *(/ĕ/)*
2. What mark do we use to show a short vowel? *(A breve ˘ — like a smile)*
3. Say a word with short /ĕ/ that we haven't used today.

---

**Next lesson:** Lesson 7: VC Words

---

*Practice at home: Find 5 things in your house with the short /ĕ/ sound.*
