# Lesson 4: Short Vowel O — /ŏ/

**Stage 2** · Lesson 4 · vowel-concept

---

## Warm-Up: Phonogram Flash Review

> Flash all 26 a-z phonograms. Child says ALL sounds within 2 seconds. Focus especially on **o**.

| Review all a-z |
|----------------|
| a, d, g, c, o, qu, s, t, i, p, u, j, r, n, m, e, l, b, h, k, f, v, w, x, y, z |

---

## New Learning: The Short Sound of **O**

### Meet /ŏ/

**O** has multiple sounds. The short sound of **o** is /ŏ/.

The short sound of O is /ŏ/ — like the first sound in 'octopus.' Your mouth makes a round circle. Imagine you're surprised: 'O!' but shorter. Say it: /ŏ/.

<div class="phonogram">o</div>

### The Breve Mark ˘

When o says its short sound /ŏ/, we can mark it with a **breve** (˘) — a little smile above the letter. This helps us remember it's the short sound.

> ō  = long &nbsp;&nbsp;|&nbsp;&nbsp; ŏ = short (breve)

### Short O Words

| Word | Sounds | Meaning |
|------|--------|---------|
| hot | /h/ /o/ /t/ | — |
| pot | /p/ /o/ /t/ | — |
| not | /n/ /o/ /t/ | — |
| got | /g/ /o/ /t/ | — |
| dot | /d/ /o/ /t/ | — |
| lot | /l/ /o/ /t/ | — |
| rot | /r/ /o/ /t/ | — |
| cot | /c/ /o/ /t/ | — |

---

## Spelling Analysis

| Word | Phonograms Used | Rules Applied | Say-to-Spell |
|------|----------------|---------------|--------------|
| hot | h (/h/), o (/ŏ/), t (/t/) | Short vowel — closed syllable | /hot/ |
| pot | p (/p/), o (/ŏ/), t (/t/) | Short vowel — closed syllable | /pot/ |
| not | n (/n/), o (/ŏ/), t (/t/) | Short vowel — closed syllable | /not/ |

> **At this stage:** The teacher may need to help by saying each sound slowly. Child writes one phonogram at a time, saying the sound as they write.

---

## Reading Practice

Read these words sound by sound, then blend:

> hot &nbsp;&nbsp; pot &nbsp;&nbsp; not &nbsp;&nbsp; got &nbsp;&nbsp; dot &nbsp;&nbsp; lot &nbsp;&nbsp; rot &nbsp;&nbsp; cot &nbsp;&nbsp; jot &nbsp;&nbsp; top &nbsp;&nbsp; hop &nbsp;&nbsp; pop &nbsp;&nbsp; mop &nbsp;&nbsp; cop &nbsp;&nbsp; bop &nbsp;&nbsp; dog &nbsp;&nbsp; log &nbsp;&nbsp; fog &nbsp;&nbsp; hog &nbsp;&nbsp; jog &nbsp;&nbsp; bog &nbsp;&nbsp; on &nbsp;&nbsp; ox &nbsp;&nbsp; box &nbsp;&nbsp; fox &nbsp;&nbsp; sox &nbsp;&nbsp; mom &nbsp;&nbsp; Tom &nbsp;&nbsp; rod &nbsp;&nbsp; nod &nbsp;&nbsp; sod &nbsp;&nbsp; pod &nbsp;&nbsp; cod &nbsp;&nbsp; god

---

## Dictation

Adult reads these sentences aloud. Child writes them.

> The hot is big.

---

## Quick Check

1. What is the short sound of **o**? *(/ŏ/)*
2. What mark do we use to show a short vowel? *(A breve ˘ — like a smile)*
3. Say a word with short /ŏ/ that we haven't used today.

---

**Next lesson:** Lesson 5: Short Vowel u

---

*Practice at home: Find 5 things in your house with the short /ŏ/ sound.*


