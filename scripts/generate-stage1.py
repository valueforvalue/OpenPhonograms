...
"""Generate all 48 Stage 1 lesson markdown files via Jinja templates.

Architecture (slice 1 of #22 + #23):
  - Phonogram data lives in data/phonograms.yaml; loaded via framework.data_loader.
  - Stage-1-specific data (writing instructions, spelling/reading word lists,
    review match words) stays in this module as plain Python data.
  - Lesson scaffolds live in templates/stage-1/*.md.j2.
...
TEMPLATE_DIR = PROJECT_ROOT / "templates" / "stage-1"
...
# Stage 1 teaching order (matches original PHONOGRAMS dict sequence).
STAGE1_ORDER = ["a", "d", "g", "c", "o", "qu", "s", "t", "i", "p", "u", "j",
                "r", "n", "m", "e", "l", "b", "h", "k", "f", "v", "w", "x", "y", "z"]

_PHONOGRAMS = {p.id: p for p in load_phonograms() if p.stage == 1}
# Reorder to teaching sequence (data_loader returns sorted).
_PHONOGRAMS = {pid: _PHONOGRAMS[pid] for pid in STAGE1_ORDER if pid in _PHONOGRAMS}

# Spelling words (say-to-spell breakdowns) per Stage 1 phonogram.
SPELLING_WORDS: dict[str, list[tuple[str, str, str, str]]] = {
...
def spelling_words_for(phonogram: str) -> list[tuple[str, str, str, str]]:
    return SPELLING_WORDS.get(phonogram, [("—", "—", "—", "—")])


# Reading practice words per Stage 1 phonogram.
READ_WORDS: dict[str, list[str]] = {
    "a": ["at", "am", "an", "ad", "ag"],
...
def get_read_words(phonogram: str, limit: int = 6) -> list[str]:
    return READ_WORDS.get(phonogram, [])[:limit]


# Match words for review lessons (word → first-sound → first-card).
MATCH_WORDS: dict[str, tuple[str, str, str]] = {
    "a": ("apple", "/ă/", "a"),
    "d": ("dog", "/d/", "d"),
    "g": ("goat", "/g/", "g"),
    "c": ("cat", "/k/", "c"),
...
def title_for(num: int, pg_list: list[str]) -> str:  # noqa: ARG001
    return _LESSON_TITLES.get(num, f"Lesson {num}")


def next_for(num: int, all_pgs: list[str]) -> tuple[int, str]:
    """Return (next_num, next_title) — naive, used as fallback."""
    return num + 1, f"Lesson {num + 1}"


def build_phonogram_intro(num: int, pg: str, next_num: int, next_title: str) -> str:
    pg_data = _PHONOGRAMS[pg]
    sounds = pg_data.sounds
    first_sound = sounds.split("/")[1].split("/")[0]
    sc = pg_data.sound_count
    plural = "s" if sc > 1 else ""

    example_rows = "\n".join(f"| /{e[0]}/ | {e[1]} |" for e in pg_data.examples)
    writing_steps = "\n".join(f"{i+1}. {s}" for i, s in enumerate(pg_data.writing))

    if pg_data.rule:
        rule_note = f"\n> **Spelling Rule:** {pg_data.rule}\n"
    elif pg == "c":
    ...
    taught_order = list(_PHONOGRAMS.keys())
...
"""
    ...
    return env.get_template("phonogram-intro.md.j2").render(
        lesson_num=num,
...
def build_pa(num: int, title: str, desc: str, activities: list, movement: str,
             quick_check: str, home_practice: str, next_num: int, next_title: str) -> str:
    return env.get_template("phonemic-awareness.md.j2").render(
        lesson_num=num,
        title=title,
...
def build_pa_with_review(num: int, title: str, desc: str, activities: list,
                         movement: str, quick_check: str, home_practice: str,
                         review_pgs: list[str], next_num: int, next_title: str) -> str:
    return env.get_template("phonemic-awareness-with-review.md.j2").render(
        lesson_num=num,
        title=title,
        activity_description=desc,
        review_phonograms=", ".join(review_pgs),
        activities=activities,
        movement_activity=movement,
        quick_check=quick_check,
        home_practice=home_practice,
...
def build_review(num: int, group_num, group_pgs: list[str], all_pg_list: list[str],
                 next_num: int, next_title: str) -> str:
    count = len(group_pgs)
    sound_hunt = "\n".join(f"- Find the phonogram that says **{_PHONOGRAMS[p].sounds}**" for p in group_pgs)
    write_sounds = ", ".join(_PHONOGRAMS[p].sounds.split()[0] for p in group_pgs)
    match_rows = "\n".join(
...
def build_assessment() -> str:
    phonogram_checklist = "\n".join(
        f"| {p} | {_PHONOGRAMS[p].sounds} | ☐ |" for p in _PHONOGRAMS
    )
    write_checklist = (
        "| /ă/ | a | ☐ |\n| /d/ | d | ☐ |\n| /g/ | g | ☐ |\n| /k/ | c | ☐ |\n"
        "| /ŏ/ | o | ☐ |\n| /kw/ | qu | ☐ |\n| /s/ | s | ☐ |\n| /t/ | t | ☐ |\n"
...
# ── Lesson sequencing (identical to pre-refactor) ──────────────────────────
...
def generate_all():
    ...
    yield 1, build_pa(1, "Sounds Around Us",
    ...
    yield 2, build_pa(2, "Voiced and Unvoiced Sounds",
    ...
    yield 3, build_pa(3, "Blending Compound Words",
    ...
    yield 4, build_pa(4, "Blending Syllables",
    ...
    yield 5, build_pa(5, "Onset and Rime",
    ...
    yield 6, build_pa(6, "Blending CVC Words",
    ...
    yield 7, build_pa(7, "Segmenting CVC Words",
    ...
    yield 8, build_pa(8, "First Sounds in Words",
    ...
    for i, (pg, num) in enumerate(group1):
        ...
        yield num, build_phonogram_intro(num, pg, next_num, next_title)
    ...
    yield 23, build_pa_with_review(23, "Last Sounds in Words",
    ...
    yield 31, build_pa_with_review(31, "Middle Sounds in Words",
    ...
    yield 39, build_phonogram_intro(39, "y", 40, "Phonogram z")
    yield 40, build_phonogram_intro(40, "z", 41, "Sound Swapping Game")

    all_26 = ["a","d","g","c","o","qu","s","t","i","p","u","j","r","n","m","e","l","b","h","k","f","v","w","x","y","z"]
    yield 41, build_pa_with_review(41, "Sound Swapping Game",
    ...
    yield 42, env.get_template("vowel-concept.md.j2").render(
    ...
    yield 44, build_pa_with_review(44, "Blending with Consonant Blends",
    ...
    yield 45, build_pa_with_review(45, "Blending Two-Syllable Words",
    ...
    yield 46, env.get_template("handwriting.md.j2").render(
        ...
        letter_group="## Clockface Letters\n\nThese letters all start with a curve — like the face of a clock! The letters are: **a, d, g, c, o, qu, s**",
        letter_list="",
    ...
    yield 47, env.get_template("handwriting.md.j2").render(
...
def main():
    ...
    for num, content in generate_all():
        if content is None:
            continue
        slug = LESSON_SLUGS.get(num, f"lesson-{num:03d}")
        filepath = OUT_DIR / f"{slug}.md"
        # Match pre-refactor byte layout. Phonogram-intro lessons (with
        # teacher_script injected) end with a blank line; PA/review/etc.
